# 原子記憶系統 Atomic Memory V2.21

> **Claude Code 跨 Session 知識管理引擎**
> Hybrid RECALL | Dual LLM | Workflow Guardian | Write Quality Gate | Wisdom Engine | Fix Escalation | Failures 自動化 | Token Diet | ACT-R Activation | Read Tracking | Rules 模組化 | 自我迭代自動化 | 覆轍偵測 | Section-Level 注入 | 路徑集中化（wg_paths）| Project Registry | 專案自治層
...
腳本自動完成：npm 全域套件（MCPControl、Playwright）、`~/.claude.json` MCP 設定、IDENTITY/USER 初始化。
詳細步驟（Python 套件、Ollama 模型、手動合併安裝）→ [Install-forAI.md](Install-forAI.md)
...
│
├── hooks/
│   ├── workflow-guardian.py          ← 統一 Hook 入口 (~1130 行, dispatcher)
│   ├── wg_paths.py                  ← 路徑唯一真相來源 (~290 行) [V2.20]
│   ├── wg_core.py                   ← 共用常數/設定/state IO (~266 行)
│   ├── wg_atoms.py                  ← 索引/trigger/ACT-R/Section 注入 (~403 行)
│   ├── wg_intent.py                 ← 意圖分類/向量搜尋 (~340 行)
│   ├── wg_extraction.py             ← 回應萃取/worker 管理 (~257 行)
│   ├── wg_episodic.py               ← Episodic 生成/衝突偵測 (~863 行)
│   ├── wg_iteration.py              ← 自我迭代/衰減/覆轍 (~415 行)
│   ├── extract-worker.py            ← LLM 萃取子程序 (~530 行，detached)
...
│
├── {project_root}/.claude/          ← 專案自治層 (V2.21, 每專案獨立)
│   ├── memory/MEMORY.md            ← 專案 atom 索引
│   ├── memory/*.md                 ← 專案 atoms
│   ├── memory/failures/            ← 專案踩坑記錄
│   ├── memory/episodic/            ← 自動生成 (gitignore)
│   ├── memory/_staging/            ← 暫存 (gitignore)
│   ├── hooks/project_hooks.py      ← 專案 delegate
│   └── .gitignore                  ← 排除 ephemeral 檔案
│
...
### Vanilla Claude Code vs + Atomic Memory V2.21

| 指標 | Vanilla Claude Code | + Atomic Memory V2.21 |
|------|--------------------|-----------------------|
