...
記憶檔採用 atom 格式，每個檔案包含：
- **元資料**：Scope / Confidence / Source / Last-used / Trigger / Privacy
- **知識段落**：核心事實
- **行動段落**：遇到觸發條件時該怎麼做

auto-memory 目錄中的 `MEMORY.md` 作為 **Atom Index**：只放索引 + 高頻事實（~30 行以內），分類 atom 檔按需載入。載入判斷依據各 atom 的 Trigger 欄位。
...
- **常態載入最小化，細節按需深入**
- `MEMORY.md` 只放索引 + 高頻事實，不堆積歷史細節
- 新增記憶前自問：「這是每個 session 都需要的嗎？」— 不是就放分類 atom 檔
...
> 「這次修改涉及 N 個檔案，要我同步更新 _CHANGELOG、MEMORY 和 git 嗎？」

確認後依序執行：

1. **_CHANGELOG 更新**：在 `_AIDocs/_CHANGELOG.md` 追加變更記錄（超過 8 筆時觸發滾動淘汰）
2. **記憶更新**：
   - auto-memory 的 atom 檔：更新相關 atom 的知識段落 + Last-used 日期
   - 新的決策 → 更新 `Extra_Efficiently_TokenSafe.md`（含演化日誌）
   - 新的坑點 → 更新對應 atom 檔
3. **Git 同步**（若專案有 git repo）：
   - 將 workspace 變更複製到 repo 目錄
   - 秘密洩漏檢查（grep 真實 token/ID，確保 repo 只有 `{{PLACEHOLDER}}`）
   - `git add` → `git commit` → `git push`

**不要等使用者提醒**，這是 AI 的主動責任。
**每一步都要做**，不要只做記錄不做 git，或只做 git 不做記錄。
