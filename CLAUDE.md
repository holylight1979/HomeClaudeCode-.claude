...
### 自我迭代原則（Self-Iteration V2.6）

原子記憶系統自身也隨使用不斷深化演進。以下 8 條原則由跨學科理論背書（詳見 `memory/openclaw-self-iteration.md`）。

#### 三維演進目標

| 維度 | 演進方向 | 理論基礎 |
|------|---------|---------|
| **精確度** | 發現更好的判斷模式 → 更新行動規則 | Aristotle episteme |
| **協助力** | 識別使用者未明說但反覆需要的支援模式 | Aristotle techne |
| **良善性** | 降低認知負擔、減少來回確認的摩擦 | Aristotle phronesis |

#### 8 條行動規則

1. **品質函數**：每次互動產生三類訊號 — 使用者確認（+）、糾正（−）、無回饋（0）。確認 → Confirmations+1，糾正 → 更新規則，無回饋 → 不動作
2. **收斂優先**：新增 atom 知識前，先檢查是否有可合併的既有規則。規則總數趨向收斂，不無限膨脹
3. **證據門檻**：≥2 次獨立 session 觀察才建立正式規則。單次觀察只記 [臨]（hook 自動追蹤）
4. **淘汰勇氣**：每新增 1 條知識，主動檢查是否有 1 條可淘汰。寧減不加（Via negativa）
5. **震盪偵測**：同一 atom 在 3 session 內被修改 2+ 次 → 暫停修改，等更多證據（hook 自動偵測，SessionEnd 報告）
6. **成熟度感知**：系統有學習期/穩定期/成熟期。成熟期減少新規則，專注精煉（hook 自動計算，SessionStart 報告）
7. **三維平衡**：迭代時同時考量精確度/協助力/良善性，不偏廢任一維
8. **演進邊界**：自我迭代**只能**更新「行動」和「知識」段落。**絕對禁止**自行修改 Confidence 層級（晉升仍需使用者確認或跨 session 鞏固機制）

#### 定期檢閱（自動觸發）

SessionStart 時 hook 自動檢查距上次檢閱的 session 數（預設每 6 session）。觸發時注入提醒，Claude 執行：
1. 掃描近期 episodic atoms + knowledge_queue
2. 找出重疊性高的使用者要求模式
3. 反覆出現的要求 → 收攏為 `[觀]` 或晉升為 `[固]`
4. 更新向量資料庫
5. 完成後呼叫 `_save_review_marker()` 重置計數器（透過寫入 `workflow/last_review_marker.json`）
