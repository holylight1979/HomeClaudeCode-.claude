...
> Atomic Memory System V2.7 Specification
> 適用於 Claude Code 跨 session 知識管理。
> Hybrid RECALL + Write Gate + 回應知識捕獲 + 自我迭代引擎
...
| 使用者明確拒絕 | 移入遙遠記憶 | 同左 |
| TTL 設定且已過期 | 依 confidence 處理 | [臨] 自動移入；[觀][固] 標記 |
...
- Privacy: public
- Tags: pitfall, architecture
- Related: other-atom-name
- Supersedes: old-atom-name
- Quality: 0.85
...
| Created | | （空） | 首次建立日期 |
| Confirmations | | 0 | session 引用且未推翻時 +1 |
| TTL | | null | 自訂存活期，null = 由 confidence 決定 |
...
TYPE_DECAY_MULTIPLIER = {'semantic': 1.0, 'episodic': 0.8, 'procedural': 1.5}
...
| searcher.py | 同目錄 | 語意搜尋 + ranked search |
...
- SessionEnd hook 自動觸發增量索引（若 session 中有 atom 修改）
...
├─ intent classification (rule-based, ~1ms)
├─ HTTP → Vector Service /search/ranked (~200-500ms, timeout 2s)
│   └─ FinalScore = 0.45×Semantic + 0.15×Recency + 0.20×IntentBoost
│                    + 0.10×Confidence + 0.10×(Confirmation + TypeBonus)
├─ Keyword Boost: 大寫詞/引號短語/中文專有名詞 → 雙命中 +0.1 | 救回 +0.05
├─ merge & deduplicate (keyword 優先)
├─ Supersedes filter: 被取代的 atom 不載入
├─ Related auto-load: 載入關聯 atom 摘要
└─ load atoms within token budget
    └─ Token budget: len(prompt)<50 → 1500t; <200 → 3000t; else → 5000t
    └─ Per-atom cost: len(content) // 4 (char-to-token estimate)
```
...
| 2.0 | 2026-03-03 | Hybrid RECALL（keyword + vector + LLM） |
| 2.1 | 2026-03-04 | Schema 擴展、Write Gate、Intent ranking、衝突偵測、Audit Trail |
| 2.4 | 2026-03-05 | 回應知識捕獲（逐輪+SessionEnd LLM 萃取）+ 跨 Session 鞏固 |
| 2.5 | 2026-03-06 | Keyword Boost + Self-healing Collection Cache |
| 2.6 | 2026-03-08 | Self-Iteration Engine（品質函數 + 震盪偵測 + 成熟度模型） |
| 2.7 | 2026-03-10 | CLAUDE.md 精簡 50% + SPEC 清理（移除版本標註、合併元資料表格） |
