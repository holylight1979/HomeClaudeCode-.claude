...
- Related: toolchain, feedback-global-install, _INDEX
...
- [固] Python subprocess stdout 在 Windows 預設 cp950 → Node.js exec 當 UTF-8 解碼 → Dashboard 亂碼 → Python 開頭強制 `sys.stdout = TextIOWrapper(sys.stdout.buffer, encoding='utf-8')` + Node exec env 注入 `PYTHONIOENCODING=utf-8`（根因: console codepage cp950，Python sys.stdout.encoding 繼承系統 codepage；設過 `chcp 65001` 的機器不會重現）
